...
router.get('/achievements', requireAuth(), async (req, res, next) => {
  try {
    ...
    if (streak && streak.currentStreak >= 3) {
      achievements.push({ id: 'streak_3', name: '3-Day Streak', icon: '🔥', unlocked: true });
    }
    if (streak && streak.currentStreak >= 7) {
      achievements.push({ id: 'streak_7', name: '7-Day Streak', icon: '🔥🔥', unlocked: true });
    }
    if (streak && streak.currentStreak >= 30) {
      achievements.push({ id: 'streak_30', name: '30-Day Streak', icon: '🔥🔥🔥', unlocked: true });
    }

    // Quiz achievements
    if (quizCount >= 1) {
      achievements.push({ id: 'quiz_first', name: 'First Quiz', icon: '🎯', unlocked: true });
    }
    if (quizCount >= 10) {
      achievements.push({ id: 'quiz_10', name: 'Quiz Master', icon: '🎓', unlocked: true });
    }

    // Flashcard achievements
    if (flashcardCount >= 1) {
      achievements.push({ id: 'flashcard_first', name: 'First Flashcard Set', icon: '📇', unlocked: true });
    }
    if (flashcardCount >= 5) {
      achievements.push({ id: 'flashcard_5', name: 'Flashcard Pro', icon: '📚', unlocked: true });
    }

    // Study time achievements
    if (streak && streak.totalStudyTime >= 3600) {
      achievements.push({ id: 'time_1h', name: '1 Hour Studied', icon: '⏱️', unlocked: true });
    }
    if (streak && streak.totalStudyTime >= 36000) {
      achievements.push({ id: 'time_10h', name: '10 Hours Studied', icon: '⏰', unlocked: true });
    }
...
router.get('/dashboard', requireAuth(), async (req, res, next) => {
  try {
    ...
    const activityByDate = recentActivity.reduce((acc: any, session) => {
